...
class CardPayPaymentHttpResponse extends EPaymentHmacSignedMessage implements IEPaymentHttpPaymentResponse
{
    protected $isVerified = false;

    const ECDSA_KEY_LIST_URL = "https://moja.tatrabanka.sk/e-commerce/ecdsa_keys.txt";

    /**
     * CardPayPaymentHttpResponse constructor.
     * @param null $fields
     */
    public function __construct($fields = null)
    {

        $this->readOnlyFields = ["AMT", "CURR", "VS", "TXN", "RES", "AC", "TRES", "RC", "TID", "TIMESTAMP", "HMAC", "ECDSA_KEY", "ECDSA"];
...
-----END PUBLIC KEY-----";
    ...
    {
        return "{$this->AMT}{$this->CURR}{$this->VS}{$this->TXN}{$this->RES}{$this->AC}{$this->TRES}{$this->CID}{$this->RC}{$this->TID}{$this->TIMESTAMP}{$this->HMAC}";
    }
    ...
    {
        if (empty($this->VS)) {
            throw new EPaymentException("Variable symbol is empty");
        }

        if (!($this->RES == "FAIL" || $this->RES == "OK")) {
            throw new EPaymentException("Unknown repsonse code");
        }
    }

    protected function getSignatureBase()
    {
        return "{$this->AMT}{$this->CURR}{$this->VS}{$this->TXN}{$this->RES}{$this->AC}{$this->TRES}{$this->CID}{$this->RC}{$this->TID}{$this->TIMESTAMP}";
    }
}